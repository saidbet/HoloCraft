...
public class NetworkSpawnManager : SpawnManager<SyncSpawnedObject>
{
    ...
    protected override void InstantiateFromNetwork(SyncSpawnedObject spawnedObject)
    {
        ...
        if (spawnedObject.ownerId.Value != thisUser)
        {
            ...
            else if (spawnedObject.id!=null)
                prefab = CloudDataManager.Instance.GetPrefab(spawnedObject.id.Value);
    ...
    public GameObject SpawnFromCache(SyncSpawnedObject dataModel, int verandaId, int userGroup, GameObject parent)
    {
        GameObject prefab = CloudDataManager.Instance.GetPrefab(verandaId);
        ...
        if (SyncSource == null)
        {
            GameObject instance = Instantiate(CloudDataManager.Instance.GetPrefab(verandaId));
            return instance;
        }
        ...
        dataModel.Transform.Scale.Value = prefab.transform.localScale;

        string instanceName = CreateInstanceName(prefab.name);

        User owner = SharingStage.Instance.Manager.GetLocalUser();

        dataModel.Initialize(instanceName, parent.transform.name, verandaId, userGroup, owner.GetID());
